...
<div id="app" v-cloak>
    ...
    <div class="uk-margin uk-flex uk-flex-between uk-flex-wrap">
        <div class="uk-flex uk-flex-middle uk-flex-wrap">
            ...
            <h2 class="uk-h3 uk-margin-remove" v-if="!selected.length">
                {{ '{0} %count% Categories|{1} %count% Category|]1,Inf[ %count% Categories' | transChoice(count, {count: count})
                }}</h2>

            <template v-else>
                <h2 class="uk-h3 uk-margin-remove">
                    {{ '{1} %count% Category selected|]1,Inf[ %count% Categories selected' | transChoice(selected.length, {count: selected.length})
                    }}</h2>
                ...
                <div class="uk-margin-left">
                    <ul class="uk-iconnav">
                        ...
                        <li><a uk-icon="icon:trash;ratio:1" :uk-tooltip="'Delete' | trans" @click="remove"
                               v-confirm="'Delete Categories?'"></a></li>
                    </ul>
                </div>
            </template>
        ...
        <div>
            <a class="uk-button uk-button-primary"
               :href="$url.route('admin/blog/categories/edit')">{{ 'Add Category' | trans }}</a>
        </div>
    </div>
    ...
    <div class="uk-overflow-auto">
        <table class="uk-table uk-table-hover uk-table-middle">
            ...
            <tr>
                ...
                <th class="uk-width-large" v-order:title="config.filter.order">{{ 'Title' | trans }}</th>
            ...
            <tbody>
            <tr class="check-item" v-for="category in categories" :key="category.id"
                ...
                <td class="pk-table-text-break">
                    <a target="_blank" v-if="category.accessible && category.url"
                       :href="$url.route(category.url.substr(1))">{{ decodeURI(category.url) }}</a>
    ...
    <h3 class="uk-h2 uk-text-muted uk-text-center" v-show="categories && !categories.length">
        {{ 'No categories found.' | trans }}</h3>
