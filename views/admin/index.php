...
<div id="app" v-cloak>
    ...
    <div class="uk-margin uk-flex uk-flex-between uk-flex-wrap">
        <div class="uk-flex uk-flex-middle uk-flex-wrap">
            ...
            <v-title v-if="!selected.length"
                     :title="'{0} %count% Posts|{1} %count% Post|]1,Inf[ %count% Posts' | transChoice(count, {count:count})"></v-title>

            <template v-else>
                <v-title
                        :title="'{1} %count% Post selected|]1,Inf[ %count% Posts selected' | transChoice(selected.length, {count:selected.length})"></v-title>
                <div class="uk-margin-left">
                    <ul class="uk-iconnav">
                        ...
                        <li><a uk-icon="icon:trash;ratio:1" :uk-tooltip="'Delete' | trans" @click="remove"
                               v-confirm="'Delete Posts?'"></a></li>
                    </ul>
                </div>
            </template>
        ...
        <div>
            ...
            <a class="uk-button uk-button-primary" :href="$url.route('admin/blog/post/edit')">{{ 'Add Post' | trans
                }}</a>
    ...
    <div class="uk-overflow-auto">
        <table class="uk-table uk-table-hover uk-table-middle">
            <thead>
            <tr>
                <th class="pk-table-width-minimum"><input class="uk-checkbox" type="checkbox"
                                                          v-check-all:selected="{ selector: 'input[name=id]' }" number>
                </th>
                <th class="pk-table-min-width-200" v-order:title="config.filter.order">{{ 'Title' | trans }}</th>
                <th class="pk-table-width-100 uk-text-center">
                    <input-filter :title="$trans('Categories')" :value.sync="config.filter.category"
                                  :options="categoryOptions" v-model="config.filter.category"></input-filter>
                </th>
                <th class="pk-table-width-100 uk-text-center">
                    <input-filter :title="$trans('Status')" :value.sync="config.filter.status" :options="statusOptions"
                                  v-model="config.filter.status"></input-filter>
                </th>
                <th class="pk-table-width-100">
                    <span v-if="!canEditAll">{{ 'Author' | trans }}</span>
                    <input-filter :title="$trans('Author')" :value.sync="config.filter.author" :options="users"
                                  v-model="config.filter.author" v-else></input-filter>
                </th>
            ...
            <tr class="check-item" v-for="post in posts" :key="post.id" :class="{'uk-active': active(post)}">
                ...
                <td class="uk-text-center">
                    <span v-if="post.categories.length > 3" class="uk-text-meta" style="cursor:help"
                          :uk-tooltip="getCategoriesText(post.categories)">{{'Categories' | trans}}</span>
                    <a v-else v-for="category in post.categories" :key="category.id"
                       :href="$url.route('admin/blog/categories/edit', { id: category.id })" target="_blank">
                        {{category.title}}
                    </a>
                </td>
                <td class="uk-text-center">
                ...
                <td class="pk-table-text-break">
                    <a target="_blank" v-if="post.accessible && post.url"
                       :href="$url.route(post.url.substr(1))">{{ decodeURI(post.url) }}</a>
